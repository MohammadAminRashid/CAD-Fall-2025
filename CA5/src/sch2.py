...
class ListScheduler(ABC):
    ...
    # Helper to check if a node is already scheduled
    ...
    # Helper to collect all operators in the tree for termination check
...
class MinLatencyScheduler(ListScheduler):
    ...
    def find_candidate_nodes(self) -> List[OperatorNode]:
        ...
        for op in all_ops:
            ...
            # Check if all operands are ready (either Identifiers or scheduled Operators)
            is_ready = True
    ...
    def select_from_frontier(self, frontier : List[OperatorNode]) -> List[OperatorNode]:
        # Hu's Algorithm: Priority is depth (path length). 
        # In dfg_creator, 'depth' is distance from root. The 'input' leaves have MAX depth.
        # We want to schedule leaves first, so higher depth = higher priority.
        
        # Sort candidates by depth descending (Hu's priority)
        frontier.sort(key=lambda x: x.depth, reverse=True)
        
        selected = []
        # Track resource usage for this cycle
        # We assume self.numof_resources keys are like "ALU", "MUL"
    ...
    def schedule(self) -> None:
        ...
        while len(self.scheduled_nodes_info) < len(all_ops):
            ...
            if not frontier:
                break # Should not happen if graph is valid and incomplete
                
            selected_nodes = self.select_from_frontier(frontier)
            
            # Group by type to assign resource numbers (1, 2, ...) correctly
...
class MinResourceScheduler(ListScheduler):
    ...
    def find_candidate_nodes(self) -> List[OperatorNode]:
        ...
        self._get_all_operators(self.root, all_ops)
        for op in all_ops:
            if not self._is_scheduled(op):
                if all(not isinstance(c, OperatorNode) or self._is_scheduled(c) for c in op.operands):
                    candidates.append(op)
        return candidates

    def select_from_frontier(self, frontier : List[OperatorNode], current_time : int) -> List[OperatorNode]:
        # Sort by Slack (ALAP - current_time)
        # Nodes with smaller slack are prioritized
        frontier.sort(key=lambda x: self.latest_time.get(x.id, self.max_time) - current_time)
        
        selected = []
        # Temporary tracker for resources used IN THIS CYCLE
        ...
        for node in frontier:
            ...
            # Check if we have an available resource already allocated in self.numof_resources
            if usage_this_cycle[res_type] < self.numof_resources.get(res_type, 0):
                selected.append(node)
                usage_this_cycle[res_type] += 1
            
            # THE KEY CHANGE: If slack is ZERO and we have no resource, ADD one!
            elif slack <= 0:
                print(f"[Resource Management] Adding 1 {res_type} because node {node.id} has 0 slack.")
                # Increase the global resource count for this design
    ...
    def schedule(self) -> None:
        ...
        while len(self.scheduled_nodes_info) < len(all_ops):
            ...
            # Record using the resource indices (1, 2, 3...)
            type_counters = {k: 1 for k in OP_TYPES}
            for node in selected_nodes:
                res_type = node.op_type
                res_num = type_counters[res_type]
